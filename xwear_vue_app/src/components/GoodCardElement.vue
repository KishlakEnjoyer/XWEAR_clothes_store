    ...
    async fetchGoodData() {
      ...
      try {
        const response = await fetch(
          `http://localhost:5289/api/Good/${this.article}`
        );
...
<template>
  <div v-if="goodData" class="good-card">
    ...
    <div v-else>
      ...
      <img
        :src="getImageUrl(goodData.imagePath)"
        :alt="`${goodData.brand?.name || 'Товар'} ${
          goodData.model?.name || ''
        }`"
      />

      <RouterLink id="good-name" style="text-decoration: none;" 
      :to="{ name: 'product', params: { articleGood: goodData.article } }">
        {{ goodData.article }}
        {{ goodData.brandName || "Неизвестный бренд" }}
        {{ goodData.modelName || "Неизвестная модель" }}
      </RouterLink>
  ...
  <div v-else>Товар не найден или данные недоступны.</div>
</template>
...
.good-card svg {
  ...
  &:hover {
    ...
    filter: drop-shadow(0 0 8px rgba(255, 255, 255, 1))
      drop-shadow(0 0 12px rgba(0, 255, 234, 0.8));
    color: #72cdfa;
  }
}
