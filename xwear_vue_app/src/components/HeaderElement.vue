...
const API_BASE_URL = "http://localhost:5289/api/User";
...
const handleLogin = async () => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      error.value = errorData.message || "Неверный логин или пароль";
    }
  } catch (err) {
...
const handleRegister = async () => {
  ...
  // if(registerData.value.phone.length > 13 || registerData.value.phone.length < 11){
  //   error.value = "Номер телефона некорректен!";
  //   return;
  // }
  const phoneNumber = parsePhoneNumber(registerData.value.phone, 'RU');
...
const openProfile = () => {
  ...
  profileData.value.nickname = currentUser.value.nickname || "";
  profileData.value.phone = currentUser.value.phone || "";
...
const changeNickname = async () => {
  if (!profileData.value.nickname.trim()) {
    profileError.value = "Введите новый ник";
    return;
  }
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/change-nickname`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({
        ...
        newNickname: profileData.value.nickname,
      }),
    });
    ...
    if (response.ok) {
      ...
      currentUser.value.nickname = profileData.value.nickname;
      localStorage.setItem("currentUser", JSON.stringify(currentUser.value));
    } else {
      const errorData = await response.json();
      profileError.value = errorData.message || "Ошибка изменения ника";
    }
  } catch (err) {
...
const changePassword = async () => {
  if (
    !passwordChangeData.value.oldPassword ||
  ...
  if (
    passwordChangeData.value.newPassword !==
    passwordChangeData.value.confirmNewPassword
  ) {
    profileError.value = "Новые пароли не совпадают";
    return;
  }
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/change-password`, {
      ...
      body: JSON.stringify({
        email: currentUser.value.email,
        oldPassword: passwordChangeData.value.oldPassword,
    ...
    if (response.ok) {
      ...
      const errorData = await response.json();
      profileError.value = errorData.message || "Ошибка изменения пароля";
    }
  } catch (err) {
    profileError.value = "Ошибка соединения с сервером";
  ...
  <header>
    <div class="container">
      ...
      <nav class="header-navigation">
        ...
        <RouterLink to="/accessories" data-text="Аксессуары"
  ...
  <div v-if="isModalOpen" class="modal-overlay" @click="closeModal">
    <div ref="modalRef" class="modal" @click.stop>
      ...
      >
        ...
        <div class="form-group">
          <label for="register-nickname">Имя пользователя</label>
          <input
...
<div v-if="isProfileOpen" class="modal-overlay" @click="closeProfile">
  <div class="modal profile-modal" @click.stop>
    ...
    <div class="profile-info">
      ...
      <p><strong>Имя:</strong> {{ currentUser?.nickname || "Не указано" }}</p>
      <p><strong>Телефон:</strong> {{ currentUser?.phone || "Не указан" }}</p>
    </div>

    <div v-if="profileError" class="error-message">{{ profileError }}</div>

    <form @submit.prevent="changeNickname" class="compact-form">
      <div class="compact-row">
        <label for="profile-nickname" class="compact-label">Имя:</label>
        <input
          id="profile-nickname"
          type="text"
          ...
          required
        />
        <button type="submit" class="compact-submit-btn" :disabled="profileLoading">
          {{ profileLoading ? "..." : "Сменить" }}
        </button>
      </div>
    </form>

    <form @submit.prevent="changePassword" class="compact-form">
      <div class="compact-row">
        <input
          type="password"
...
.header-navigation a::before {
  color: #0ff;
  animation: glitch 0.3s infinite both;
  z-index: -1;
}

.header-navigation a::after {
  color: #f0f;
  animation: glitch 0.3s infinite both reverse;
  z-index: -2;
}
...
.tabs button.active {
  color: #333;
  border-bottom: 2px solid #72cdfa;
}
...
.form-group input:focus {
  outline: none;
  border-color: #72cdfa;
  box-shadow: 0 0 0 2px rgba(114, 205, 250, 0.2);
}
...
.compact-input:focus {
  outline: none;
  border-color: #72cdfa;
  box-shadow: 0 0 0 1px rgba(114, 205, 250, 0.2);
}
